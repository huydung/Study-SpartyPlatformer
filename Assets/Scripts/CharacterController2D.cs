...
public class CharacterController2D : MonoBehaviour {
	...
	bool playerCanPerformDoubleJump = false;
	...
	void Update()
	{
		...
		if (isGrounded) {
			playerCanPerformDoubleJump = true;
		}

		// Set the grounded animation states
		_animator.SetBool("Grounded", isGrounded);
		if(  Input.GetButtonDown("Jump")) // If grounded AND jump button pressed, then allow the player to jump
		{			
			if (isGrounded) {
				DoJump ();
			} else if( playerCanPerformDoubleJump ) {
				DoJump ();
				playerCanPerformDoubleJump = false;
			}
		}
